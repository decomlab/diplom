...
contract Diplom is CustomERC721Metadata {
    using SafeMath for uint256;
    ...
    mapping(uint256 => address) public projectIdToProjectOwnerAddress;
    ...
    modifier onlyProjectOwner(uint256 _projectId) {
        require(msg.sender == projectIdToProjectOwnerAddress[_projectId], "Only projectOwner");
        _;
    }
    ...
    modifier onlyProjectOwnerOrWhitelisted(uint256 _projectId) {
        require(isWhitelisted[msg.sender] || msg.sender == projectIdToProjectOwnerAddress[_projectId], "Only projectOwner or whitelisted");
        _;
    }
    ...
    function mint(address _to, uint256 _projectId, address _by) external returns (uint256 _tokenId) {
        ...
        require(projects[_projectId].active || _by == projectIdToProjectOwnerAddress[_projectId], "Project must exist and be active");
        require(!projects[_projectId].paused || _by == projectIdToProjectOwnerAddress[_projectId], "Purchases are paused.");
    ...
    function updateProjectProjectOwnerAddress(uint256 _projectId, address _projectOwnerAddress) public onlyProjectOwnerOrWhitelisted(_projectId) {
        projectIdToProjectOwnerAddress[_projectId] = _projectOwnerAddress;
    }

    function toggleProjectIsPaused(uint256 _projectId) public onlyProjectOwner(_projectId) {
        projects[_projectId].paused = !projects[_projectId].paused;
    }

    function addProject(string memory _projectName, address _projectOwnerAddress, uint256 _pricePerTokenInWei, bool _dynamic) public onlyWhitelisted {

        uint256 projectId = nextProjectId;
        projectIdToProjectOwnerAddress[projectId] = _projectOwnerAddress;
    ...
    function updateProjectCurrencyInfo(uint256 _projectId, string memory _currencySymbol, address _currencyAddress) onlyProjectOwner(_projectId) public {
        projectIdToCurrencySymbol[_projectId] = _currencySymbol;
        projectIdToCurrencyAddress[_projectId] = _currencyAddress;
    }

    function updateProjectPricePerTokenInWei(uint256 _projectId, uint256 _pricePerTokenInWei) onlyProjectOwner(_projectId) public {
        projectIdToPricePerTokenInWei[_projectId] = _pricePerTokenInWei;
    }

    function updateProjectName(uint256 _projectId, string memory _projectName) onlyUnlocked(_projectId) onlyProjectOwnerOrWhitelisted(_projectId) public {
        projects[_projectId].name = _projectName;
    }

    function updateProjectProjectOwnerName(uint256 _projectId, string memory _projectProjectOwnerName) onlyUnlocked(_projectId) onlyProjectOwnerOrWhitelisted(_projectId) public {
        projects[_projectId].projectOwner = _projectProjectOwnerName;
    }

    function updateProjectAdditionalPayeeInfo(uint256 _projectId, address _additionalPayee, uint256 _additionalPayeePercentage) onlyProjectOwner(_projectId) public {
    ...
    function updateProjectSecondaryMarketRoyaltyPercentage(uint256 _projectId, uint256 _secondMarketRoyalty) onlyProjectOwner(_projectId) public {
        require(_secondMarketRoyalty <= 100, "Max of 100%");
        projectIdToSecondaryMarketRoyaltyPercentage[_projectId] = _secondMarketRoyalty;
    }

    function updateProjectDescription(uint256 _projectId, string memory _projectDescription) onlyProjectOwner(_projectId) public {
        projects[_projectId].description = _projectDescription;
    }

    function updateProjectWebsite(uint256 _projectId, string memory _projectWebsite) onlyProjectOwner(_projectId) public {
        projects[_projectId].website = _projectWebsite;
    }

    function updateProjectLicense(uint256 _projectId, string memory _projectLicense) onlyUnlocked(_projectId) onlyProjectOwnerOrWhitelisted(_projectId) public {
        projects[_projectId].license = _projectLicense;
    }

    function updateProjectMaxInvocations(uint256 _projectId, uint256 _maxInvocations) onlyProjectOwner(_projectId) public {
    ...
    function toggleProjectUseHashString(uint256 _projectId) onlyUnlocked(_projectId) onlyProjectOwnerOrWhitelisted(_projectId) public {
    ...
    function addProjectScript(uint256 _projectId, string memory _script) onlyUnlocked(_projectId) onlyProjectOwnerOrWhitelisted(_projectId) public {
    ...
    function updateProjectScript(uint256 _projectId, uint256 _scriptId, string memory _script) onlyUnlocked(_projectId) onlyProjectOwnerOrWhitelisted(_projectId) public {
        require(_scriptId < projects[_projectId].scriptCount, "scriptId out of range");
        projects[_projectId].scripts[_scriptId] = _script;
    }

    function removeProjectLastScript(uint256 _projectId) onlyUnlocked(_projectId) onlyProjectOwnerOrWhitelisted(_projectId) public {
    ...
    function updateProjectScriptJSON(uint256 _projectId, string memory _projectScriptJSON) onlyUnlocked(_projectId) onlyProjectOwnerOrWhitelisted(_projectId) public {
        projects[_projectId].scriptJSON = _projectScriptJSON;
    }

    function updateProjectIpfsHash(uint256 _projectId, string memory _ipfsHash) onlyUnlocked(_projectId) onlyProjectOwnerOrWhitelisted(_projectId) public {
        projects[_projectId].ipfsHash = _ipfsHash;
    }

    function updateProjectBaseURI(uint256 _projectId, string memory _newBaseURI) onlyProjectOwner(_projectId) public {
        projects[_projectId].projectBaseURI = _newBaseURI;
    }

    function updateProjectBaseIpfsURI(uint256 _projectId, string memory _projectBaseIpfsURI) onlyProjectOwner(_projectId) public {
        projects[_projectId].projectBaseIpfsURI = _projectBaseIpfsURI;
    }

    function toggleProjectUseIpfsForStatic(uint256 _projectId) onlyProjectOwner(_projectId) public {
    ...
    function toggleProjectIsDynamic(uint256 _projectId) onlyUnlocked(_projectId) onlyProjectOwnerOrWhitelisted(_projectId) public {
    ...
    function overrideTokenDynamicImageWithIpfsLink(uint256 _tokenId, string memory _ipfsHash) onlyProjectOwner(tokenIdToProjectId[_tokenId]) public {
        staticIpfsImageLink[_tokenId] = _ipfsHash;
    }

    function clearTokenIpfsImageUri(uint256 _tokenId) onlyProjectOwner(tokenIdToProjectId[_tokenId]) public {
        delete staticIpfsImageLink[tokenIdToProjectId[_tokenId]];
    }

    function projectDetails(uint256 _projectId) view public returns (string memory projectName, string memory projectOwner, string memory description, string memory website, string memory license, bool dynamic) {
    ...
    function projectTokenInfo(uint256 _projectId) view public returns (address projectOwnerAddress, uint256 pricePerTokenInWei, uint256 invocations, uint256 maxInvocations, bool active, address additionalPayee, uint256 additionalPayeePercentage ,string memory currency, address currencyAddress) {
        projectOwnerAddress = projectIdToProjectOwnerAddress[_projectId];
    ...
    function getRoyaltyData(uint256 _tokenId) public view returns (address projectOwnerAddress, address additionalPayee, uint256 additionalPayeePercentage, uint256 royaltyFeeByID) {
        projectOwnerAddress = projectIdToProjectOwnerAddress[tokenIdToProjectId[_tokenId]];
